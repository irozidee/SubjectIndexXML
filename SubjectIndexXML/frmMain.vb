...
Public Class frmMain
        ...
        For Each strfilename As String In files
            ...
            For Each referredCase As String In referredCasesList
                Try
                    ...
                    If referredCitation.Contains("MLR") Then
                    ...
                    End If
                    'If subjectIndexCls.GetJudgmentNameFromCitation(referredCitation).Length = 0 Then
                    '    referredTitle = referredCase.Split({"#"}, StringSplitOptions.RemoveEmptyEntries)(1)
                    'Else
                    '    referredTitle = subjectIndexCls.GetJudgmentNameFromCitation(referredCitation)
                    'End If
    ...
    Private Sub Extract_Ref_Legislation()
        ...
        For Each Local_legis As String In Local_Legis_List
            ...
            query = "insert into ref_legislations (source_citation, source_datafilename, legis_origin, legis_title, legis_filename, legis_sub_no, legis_sub_no_type, legis_link_text) " & _
                "values('" & arr(0) & "','" & arr(1) & "','" & arr(2) & "','" & EscapeString(arr(3)) & "','" & arr(4) & "','" & Extractor.ReferredLegislations.Replace_Legislation_Sub_No_Type(arr(5)) & "','" & arr(6) & "','" & arr(7) & "')"
    ...
    Private Sub Extract_Temp_Ref_Legislation_Headnote_To_DB(ByVal XMLFilePath As String)
        ...
        For Each node As XmlNode In nodeList
            ...
            Else
                ...
                If node.InnerXml.Replace("Link", "LINK").Contains("LINK") Then
                    legis_title = node.InnerXml.Replace("Link", "LINK").Split({">", ","}, StringSplitOptions.None)(1)
                    legis_origin = "local"
                    'MsgBox(legis_title)
                    ...
                    For Each newNode As XmlNode In newNodelist
                        ...
                        Dim tmpLegisType As String = ""
                        tmpLegisType = legis_text.Replace(legis_title & ", ", "").Trim

                        Try
                            If IsNumeric(tmpLegisType(0)) Or tmpLegisType.StartsWith("(") Then
                            Else
                                legis_keyword = tmpLegisType
                            End If
                        Catch ex As Exception
                            ' MsgBox(datafilename & legis_title & " " & node.InnerText)
                        End Try

                        insertSQL = "insert into tmp_ref_leg_local(source_citation, source_datafilename, legislink_filename, legislink_no, legislink_htext, legislink_title, legislink_type, legislink_origin) " & _
                       "values('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & legis_filename & "','" & utility.Legislations.RemoveUnusedCharacter(legis_no) & _
                               "','" & EscapeString(legis_text) & "','" & EscapeString(legis_title) & "','" & legis_keyword & "','" & legis_origin & "')"
                        subjectIndexCls.Insert(insertSQL)
                    Next
                Else 'no linking legis
                    legis_origin = "foreign"
                    legis_text = node.InnerText
                    legis_keyword = "UNSPECIFIED"
                    legis_filename = ""
                    ...
                    If countComma > 1 Then ' title and many sections
                        ...
                        Dim strFSections As String = legis_text.Replace(tmpLegisTitle & ",", "").Trim
                        Dim arrFSections As String() = strFSections.Split(",")
                        For i As Integer = 0 To arrFSections.Length - 1
                            legis_title = tmpLegisTitle
                            legis_no = arrFSections(i).Trim

                            insertSQL = "insert into tmp_ref_leg_foreign(source_citation, source_datafilename, legislink_filename, legislink_no, legislink_htext, legislink_title, legislink_type, legislink_origin) " & _
                            "values('" & utility.PrintCitation(datafilename) & "','" & datafilename & "','" & legis_filename & "','" & utility.Legislations.RemoveUnusedCharacter(legis_no) & _
                            "','" & EscapeString(legis_text) & "','" & EscapeString(legis_title) & "','" & legis_keyword & "','" & legis_origin & "')"
                            subjectIndexCls.Insert(insertSQL)
                        Next
                    ...
                    Else
                        ...
                        'subjectIndexCls.Insert(insertSQL)
                        My.Computer.FileSystem.WriteAllText(log.Error_XML_Legislation, legis_text, True)
                    End If
                End If
            End If
        Next
    ...
    Private Sub Button1_Click_3(sender As Object, e As EventArgs) Handles Button1.Click
        MsgBox(Extractor.FindRegexPattern(TextBox1.Text))
    End Sub
End Class
