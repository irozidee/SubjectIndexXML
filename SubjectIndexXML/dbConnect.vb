...
#Region "SUBJECT INDEX"
    Public Function SubjectIndex_Level2(ByVal level1 As String, ByVal language As String) As List(Of String)
        Dim query As String = "SELECT level2  FROM v_subject_index where level1 = '" & EscapeString(level1) & "' and judgment_language='" & language & "' group by level2 order by level2"
    ...
    Public Function SelectSubjectIndexDataColl(ByVal level1 As String, ByVal level2 As String, ByVal summary As String) As List(Of String)
        ...
        End If
    End Function
    Public Function SubjectIndex_Level1(ByVal language As String) As List(Of String)
        Dim query As String = "SELECT level1 FROM v_subject_index where judgment_language = '" & language & "' group by level1 order by level1"
        Dim list As List(Of String) = New List(Of String)
        If Me.OpenConnection() = True Then
            Dim cmd As New MySqlCommand(query, connection)
            Dim dataReader As MySqlDataReader = cmd.ExecuteReader()
...
#Region "TABLE OF CONTENT LIST DATA"
    ...
    Public Function Table_Of_Content_Citation_Cases_Judicially(ByVal REFERRED_TITLE As String) As String
        Dim query As String = "SELECT referred_citation FROM ref_cases where referred_title = '" & EscapeString(REFERRED_TITLE) & "' group by referred_citation order by referred_citation"
        Dim list As String = ""
        If Me.OpenConnection() = True Then
            Dim cmd As New MySqlCommand(query, connection)
            'Create a data reader and Execute the command
    ...
    Public Function Table_Of_Content_Citation_Legislation_Judicially_(ByVal REFERRED_LEGIS_TITLE As String) As String
        ...
        End If
    End Function
    Public Function Table_Of_Content_Subject_Index(ByVal Language As String) As List(Of String)
        Dim query As String = "SELECT level1 FROM v_subject_index where judgment_language = '" & Language & "' group by level1 order by level1"
        Dim list As List(Of String) = New List(Of String)
        If Me.OpenConnection() = True Then
            Dim cmd As New MySqlCommand(query, connection)
        ...
        End If
    End Function
    Public Function Table_Of_Content_Citation_Subject_Index(ByVal strSubjectIndex As String, ByVal strLanguage As String) As String
        Dim query As String = "SELECT citation FROM v_subject_index where level1 = '" & EscapeString(strSubjectIndex) & "' and judgment_language='" & strLanguage & "' group by citation order by citation"
        Dim list As String = ""
        If Me.OpenConnection() = True Then
            Dim cmd As New MySqlCommand(query, connection)
            'Create a data reader and Execute the command
    ...
    Public Function CasesJudicial_Judgment_Name(ByVal firstLetter As String, ByVal title As String) As List(Of String)
        ...
        Try
            ...
            If Me.OpenConnection() = True Then
                ...
                Dim dataReader As MySqlDataReader = cmd.ExecuteReader()
                While dataReader.Read()
                    Dim col1 As String = " [" & utility.CorrectTextFormat(dataReader("judgment_name").ToString) & " " & utility.CorrectTextFormat(dataReader("citation").ToString) & "]"
...
#Region "LEGISLATION JUDICIALLY DATA"
    Public Function Legis_Judicial_Title() As List(Of String)
        ...
        If Me.OpenConnection() = True Then
            ...
            'Read the data and store them in the list
            While dataReader.Read()
                '   list(0) = dataReader("legislation") & ""
                '   list(1) = dataReader("pageno") & ""
                list.Add(utility.CorrectTextFormat(dataReader("legis_title").ToString))
            End While
    ...
    Public Function Legis_Judicial_Section(ByVal legislation As String) As List(Of String)
        Dim query As String = "SELECT legis_sub_no from v_legislations where legis_title='" & EscapeString(legislation) & "' group by legis_sub_no order by legis_sub_no" 'order by stringvalue, sortValue, section"
        Dim list As New List(Of String)
        ...
        If Me.OpenConnection() = True Then
            ...
            While dataReader.Read()
                ...
                End If
                'If dataReader("legis_sub_no").ToString = "" Then
                '    legis_sub_no_with_type = dataReader("legis_sub_type").ToString
                'Else
                '    legis_sub_no = dataReader("legis_sub_no").ToString
                '    If IsNumeric(legis_sub_no(0)) = True Then
                '        legis_sub_no_with_type = dataReader("legis_sub_type").ToString & " " & dataReader("legis_sub_no").ToString

                '    Else ' not start with number
                '        legis_sub_no_with_type = dataReader("legis_sub_no").ToString
                '    End If
                '    '    If dataReader("legis_sub_no").ToString.Contains("O ") Then
                '    '        SubNoWithType = dataReader("legis_sub_type").ToString
                '    '    Else
                '    '        SubNoWithType = dataReader("legis_sub_type").ToString & " " & dataReader("legis_sub_no").ToString
                '    '    End If
    ...
    Public Function Extract_Temp_Ref_Legislation_To_Table(ByVal referredLegisType As String) As List(Of String)
        My.Computer.FileSystem.WriteAllText("tmp_to_db_legis.txt", "", False)
        ...
        Dim query As String = "select source_citation, source_datafilename, legislink_origin, legislink_title, legislink_filename, " & _
            "legislink_no, legislink_type, legislink_htext from " & tableName
        ...
        If Me.OpenConnection() = True Then
            ...
            While dataReader.Read()
                ...
                legislink_type = Extractor.ReferredLegislations.Extract_Legislation_Sub_No_Type(dataReader("legislink_type").ToString)
                legislink_no = dataReader("legislink_no").ToString
                legislink_htext = dataReader("legislink_htext").ToString


                If legislink_type.Contains("O ") Then

                    legislink_no = legislink_type.Trim
                Else
                    ...
                    Else
                        If IsNumeric(legislink_no(0)) = True Then
                            ...
                            'CONDITON MUST GO HERE
                        Else
                            If legislink_htext.StartsWith("(") Then

                                Dim pattern As String = Extractor.FindRegexPattern(legislink_htext)
                                Dim r As New Regex(pattern, RegexOptions.Singleline)

                                Dim tmp As String = r.Replace(legislink_htext, legislink_no & legislink_htext)
                                ' MsgBox(tmp & legislink_htext)
                                legislink_no = tmp
                            Else
                                If legislink_no.StartsWith("(") Then

                                    Dim pattern As String = Extractor.FindRegexPattern(legislink_no)
                                    Dim r As New Regex(pattern, RegexOptions.Singleline)

                                    Dim tmp As String = r.Replace(legislink_type, legislink_type & legislink_no)
                                    'MsgBox(tmp & legislink_no)
                                    legislink_no = tmp
                                Else
...
#End Region
    ...
    Public Sub Extract_Temp_Ref_Legislation_DB_ToList(ByVal referredType As String) ' As List(Of String)
        ...
        If Me.OpenConnection() = True Then
            ...
            '    '   list(0) = dataReader("legislation") & ""
            '    '   list(1) = dataReader("pageno") & ""
            '    list.Add(dataReader("legislation").ToString)
            'End While
    ...
    Public Function getLegislationTitleByDatafilename(datafilename As String) As String
        ...
        If Me.OpenConnection() = True Then
            ...
            While dataReader.Read()
                ' legCitation = dataReader("datafilename").ToString().Replace(".xml", "").Replace(Environment.NewLine, "").Replace(vbLf, "").Replace(vbCr, "").Replace(vbLf & vbCr, "")
                'Exit While
                legTitle = dataReader("title").ToString
            End While
    ...
    Public Function GetJudgmentNameFromCitation(ByVal Citation As String) As String
        Dim query As String = "select title from api_cases where citation ='" & Citation & "' limit 1"
        Dim CaseTitle As String = "NO TITLE"
        If Me.OpenConnection() = True Then
            Dim cmd As New MySqlCommand(query, connection)
            Dim dataReader As MySqlDataReader = cmd.ExecuteReader()
